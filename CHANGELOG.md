...
- Map-League Phase 1 shipped (docs/NORTHSTAR_MAP_LEAGUE.md, kanban T13806138) -- a queuing
  ...
  new `FindMatchMsg` payload following `PACKET_FIND_MATCH`'s `NetHeader`, fully backward
  compatible (0/no-payload still means "no preference," matching every prior queue's random-seed
  behavior). The matchmaker (`apps/matchmaker`) honors the first requesting client's seed in
  `try_match`. Live end-to-end verified: a real matchmaker + two real clients, one requesting
  seed 424242, confirmed via the matchmaker's own log the spawned match used exactly that seed.
  Real Phase 2 (home-and-away 2-leg series) / Phase 3 (slow/fast league split) / Phase 4
  (persistent IDUNA-backed map ownership) scoped but not attempted this pass. `5ee8aed`/`4272da3`.

## 2026-09-11
- ALLCAP shipped -- the win condition itself, made mod-driven (BACKLOG.md SECTION 381). New PARENA/stdlib/ecowar/town_cap_mod.prn (trigger-only, dispatches REFLUX_ACTION_TOWN_CAPPED on a real successful town flip) + allcap_mod.prn (real, pure PARENA win-rule logic, no FFI needed: owned-count == total-count). Real 3-hop REFLUX chain: town.c's town_attempt_convert -> CAP mod dispatch -> new ecowar_tick_allcap_win_check polls + computes real counts (new town_registry_owned_count/_active_count) + asks ALLCAP mod -> dispatches ALLCAP_WIN -> arena_update/arena_update_teams poll for that to set arena_state.winner, replacing the old direct living_map_bridge_full_control_faction() call in the live match loop (that function stays for direct/test use). REFLUX extracted into its own standalone packages/reflux/ package (moved from packages/simulation) since packages/livingmap needed to dispatch into it too -- a real second internal consumer, not hypothetical. New living_map_bridge_attempt_convert_town: the real entry point a future 'capture a node' card will call. 7 new tests, full suite green (3196 assertions). Live-verified: a real fast-forwarded server + 2 bots completed a full match with the new wiring active. (sess-20260905-0720-ec33e7c5)
- REFLUX cross-mod pub/sub + real Bloodflower hostiles + day/night lighting shipped (BACKLOG.md SECTION 380). New PARENA/stdlib/reflux/reflux.prn (PARENA commit 83c1680): a real, honest cross-mod pub/sub built on a single shared append-only action log (any mod dispatches, any other mod polls) since VS0 has no function pointers/closures for a true callback dispatch table. New packages/simulation/reflux_runtime.h/.c: the real host log. Real naming collision found and fixed: PARENA-mangled reflux_dispatch/reflux_log_size collided with the runtime's own originally-planned generic names -- renamed the runtime's internal functions instead. bloodflower_mod.prn's on-moon-zenith now also dispatches REFLUX_ACTION_BLOODFLOWER_TRIGGERED (original pickup-spawn behavior unchanged); new, separate ecowar/bloodflower_hostile_spawner_mod.prn is REFLUX's first real subscriber, spawning 5 real hostile Living Map creeps (faction 3/Corruption's first live use) at map center via new living_map_bridge_spawn_hostile_creep_at_map_center + new host poller ecowar_tick_bloodflower_hostile_spawner -- neither mod calls the other directly. New arena_daynight_light_dir ports the other real half of SHANKPIT retro_lighting.c's own day/night visuals (the ambient clear-color tint was already ported; the actual 3D scene light direction apps/arena's shader uses was still a hardcoded, never-moving constant) -- now a real, moving sun-by-day/moon-by-night direction. Live-verified: a real fast-forwarded server + 2 real bots completed a full match with zero crashes; a longer headless test drove a full real day/night cycle confirming both the original pickup and the new hostiles appear. 20 new tests, full suite green (3189 assertions). Not done: SHANKPIT starfield, sky-dome rendering, visual confirmation (no display in this sandbox). (sess-20260905-0720-ec33e7c5)
- Living Map Phase 7 shipped -- real arena<->living-map live wiring (BACKLOG.md SECTION 377): founder, playing the actual live client/server: 'can we make sure we get these updates in the client and the server? hitting g doesnt bring up card casting interface - no all cap win con - im not seeing frontier village ... i dont see a hex grid.' Root cause: every Living Map system built in Phases 1-2 was real and tested but never connected to any running match. New packages/simulation/living_map_bridge.h/.c: one real, live HexGrid/TownRegistry/CreepRegistry per match, founded at arena_init/arena_init_teams (2 Frontier Villages per side, 2 neutral/contestable towns, 3 cows), ticked + win-condition-checked every arena_update/arena_update_teams tick (owner/team 0 = faction 1 Dominion, owner/team 1 = faction 2 Symbiosis). Real naming collision found and fixed: arena_game.c's own pre-existing creep_spawn (unrelated node-guardian mechanic) conflicted with packages/livingmap's -- renamed the latter to living_map_creep_spawn across creep.h/.c and every test call site, the correct permanent fix now that both systems share a binary. New PACKET_ARENA_SNAPSHOT_LIVING_MAP/ArenaSnapshotLivingMapMsg (protocol.h), sent every broadcast tick since town/creep state can't be deterministically recomputed client-side like fountains/shops. apps/arena's minimap gained real town (diamond, faction-colored) and creep/cow (yellow dot) markers -- local demo reads the bridge directly, networked path reads a client-local mirror of the new packet. Live-verified end to end: a real fast-forwarded server + two real bots completed a full match (connect/draft/pick/live/end) with the new wiring active, zero crashes. 5 new tests (tests/test_living_map_bridge.c), full suite green (3169 assertions), both build scripts clean. Not done, honestly: no hex-grid outline drawn (dots only), no card yet calls town_attempt_convert/town_apply_militia_boost so nothing can actually flip a town in a live match yet (win condition is live but currently unreachable), G-key card-UI panel redesign still not built (separate, bigger client task), no visual confirmation possible in this sandbox (no display). (sess-20260905-0720-ec33e7c5)
- Cows shipped (BACKLOG.md SECTION 377, founder: 'add cows'): real, passive, wandering neutral wildlife on the Living Map hex grid, built on top of the real creep aggro/chase/leash/reset system rather than a new one. New creep_spawn_cow (creep.h/.c): low hp, neutral faction, a new LivingMapCreep.passive flag that makes IDLE ticks wander (a plain, deterministic step pattern -- no PRNG needed) within LIVING_MAP_COW_WANDER_RADIUS hexes of home instead of scanning for aggro. A cow never fights back or initiates aggro, but isn't immune to being targeted -- its neutral faction_owner is a real, ordinary value, so a hostile faction creep's own existing aggro scan can still find and kill one for free, no special-casing needed. New LIVING_MAP_EVENT_CREEP_WANDERED event (fires only on an actual move) keeps faith with the standing 'everything announces events' requirement. 21 new tests (tests/test_cow.c) including a determinism check and a real kill-a-cow round trip. Full Living Map suite green (132 assertions). (sess-20260905-0720-ec33e7c5)
- Card-battler experiment (BACKLOG.md SECTION 377/S378) shipped: founder green-lit 'lets experiment with making the hero cards based and if it ends up being unfun we bring back classic RTS/MOBA hero affordances in order to increase player agency' -- built as a toggleable, cheap-to-revert architecture, not a wholesale replacement. New ArenaHero.npc_controlled (default 0): when set, arena_update drives that hero via new arena_npc_hero_tick (a generalized, any-hero-index merge of the existing arena_bot_tick_heuristic/bot_cast_kit_if_ready practice-bot AI -- checked directly, both were already hero-index-agnostic in their real math, only their own wrapper hardcoded hero 1 vs hero 0) instead of expecting player input; apps/arena_server's PACKET_ARENA_MOVE/PACKET_ARENA_CAST handlers both gate on it. New packages/simulation/card_deck.h/.c: real Clash-Royale-style continuous Deck/Hand mechanics (one copy of every real card id -- the 16 existing ECOWAR_CARDS plus new CARD_ID_HERO_Q/_W/_R -- 4-card hand, play empties a slot and starts a real 3s redraw). New packages/simulation/card_battler.h/.c: per-owner registry, card_battler_play_slot resolves a generic card via the existing arena_ecowar_play_card unchanged, a hero-ability card via that hero's own existing arena_cast_q/arena_toggle_w/arena_cast_r -- zero new per-hero content authored, every kit reused as-is. 24 new tests (tests/test_card_battler.c), full suite green (3145 assertions). Not built: client UI wiring (no live call sites yet), balance pass. Real rollback path kept cheap by design: npc_controlled defaults to 0, every direct-control code path is untouched. (sess-20260905-0720-ec33e7c5)
- apps/arena_server gained --tick-ms/--fast-forward flags (BACKLOG.md SECTION 377, docs/NORTHSTAR_LIVING_MAP.md's own 'ML training throughput' section) for faster real, networked bot-vs-bot self-play match generation -- founder: 'can we also make it so that ticks happen faster and in fewer frames optionally via a command line flag' for eventual ECOWAR bot unsupervised training. Both flags default to today's unchanged real-time-paced 16ms behavior (the live ecowar-matchmaker.service deploy is unaffected unless explicitly passed). --fast-forward deliberately only skips the real-time sleep once match_phase==LIVE, never during the WAITING-for-players phase -- found and fixed a real premature-shutdown risk before shipping: skipping the sleep during WAITING too would have made the existing 60-second lobby-fill timeout fire almost instantly, since a real UDP handshake still needs real wall-clock time regardless of simulation speed. Live-verified: started a real server process with both flags, confirmed it printed its real startup log and stayed alive past the WAITING timeout window rather than exiting prematurely. Also noted (not built): apps/arena_training/src/headless.c already provides a fast, in-process, zero-throttling training harness for the hero-combat side (sim_step, consumed by scripts/rl_env.py) -- the new flags close a different, real gap specific to generating training data through the actual networked matchmaker/lobby path. Real, open architecture question raised by the founder and captured, not resolved or built: converting ECOWAR's hero from directly-piloted to an NPC, making the whole game a deck-building card battler. (sess-20260905-0720-ec33e7c5)
- Living Map Phase 2 shipped (BACKLOG.md SECTION 377, docs/NORTHSTAR_LIVING_MAP.md): real classic-RTS creep aggro/chase/leash/reset (packages/livingmap/creep.h/.c, directly mirroring arena_game.h's own proven ArenaCampMinion shape -- DETECT-vs-HIT range distinction, leash measured from home not current position, reset to full hp only once actually home), a new hex_step_toward greedy-pathing helper (hex_grid.h/.c). TOWN_TYPE_WALLED_HAMLET now has real behavior (new PARENA/stdlib/ecowar/walled_hamlet_mod.prn): slower economy + lower raise threshold (defensive bias), much higher convert resistance (60 vs Frontier Village's 20 -- 'slow to flip', real and tested), and genuinely new 'shoots hostile creeps' defensive fire against the CreepRegistry (town_tick_with_creeps), stationary, garrison-scaled range/damage, real cooldown, never friendly-fires. Cards-tie-into-the-Living-Map mechanic shipped: Town.militia_bonus + town_apply_militia_boost (+1 militia/garrison raised per spawn tick per real card play, stacking, capped at 20) -- exactly matches founder's 'play it twice, villages put up three militia at a time' spec, fully tested; the real, honest gap is that no live match anywhere initializes a HexGrid/TownRegistry yet, so nothing in a live game calls this today, named as real follow-up along with the founder's own capture-node card idea (needs zero new mechanics -- town_attempt_convert's existing arbitrary attempt_strength already does exactly that). Win condition resolved (was 'genuinely undecided' in Phase 1): town_registry_faction_has_full_control -- ECOWAR's base game mode is capping every active town at once, a deliberate contrast with REDGARDEN's pace/comeback dynamic; other modes (resource race) named as later, separate work. Card-UI redesign (G-key Hearthstone/Clash-Royale-style draggable panel, drag-to-battlefield-cast vs drag-to-hexgrid-spawn) captured as real founder direction in the NORTHSTAR doc, not built this pass -- client SDL2/OpenGL work with no display available in this sandbox to verify. 43 new tests across tests/test_creep.c and tests/test_walled_hamlet.c (101 total Living Map test assertions across all 4 files), all passing via scripts/test_livingmap.sh (updated to loop over all 4 living-map test binaries) and packages/livingmap/BUILD.bazel's :living_map cc_library (still not locally verified against bazel itself). (sess-20260905-0720-ec33e7c5)
- Living Map Phase 1 shipped (BACKLOG.md SECTION 377, docs/NORTHSTAR_LIVING_MAP.md): the entire map is now formalized as a hex grid via a new, standalone packages/livingmap package (docs/NORTHSTAR_LIVING_MAP.md's own 'new layer, not an extension of the arena map' framing -- packages/simulation's existing ArenaNode MOBA sim is untouched). New hex_grid.h/.c: axial-coordinate flat-top hex math (cube-coord distance/rounding, world<->hex conversion), a fixed hex-radius-12 map (469 cells). New living_map_events.h/.c: an always-on, append-only event-announcement ring buffer every town lifecycle transition writes into (founder: 'everything that happens in the game needs to announce events') -- the NORTHSTAR doc's own 'Mod event model, honestly' section names the real, current VS0 limit (no function pointers/closures) that rules out true dynamic multi-subscriber registration today, and what's real instead: the log plus the existing call-by-name mod convention. New town.h/.c: Town struct + registry; TownType names all 4 founder-specified types (Frontier Village, Walled Hamlet, Jungle Enclave, Blighted Settlement) but only Frontier Village has real behavior this pass, per founder's own 'start with the frontier village.' New PARENA/stdlib/ecowar/frontier_village_mod.prn (real I32 decision logic, card_effect_mod.prn's own 'real decision logic, not just a trigger' tier): spawn cadence that accelerates with population, a deterministic peasant->militia promotion rule, and a real, low conversion-resistance formula matching the founder's own 'spawns peasants -> militia, avoids conflict, converts easily' spec -- generated+committed frontier_village_mod.c via the real parena build CLI, same ARENA_API.md-documented process every other mod uses. 27 new tests across tests/test_hex_grid.c and tests/test_town_frontier_village.c, all passing headlessly via new scripts/test_livingmap.sh (wired into ci.yml as its own step) and a new packages/livingmap/BUILD.bazel :living_map cc_library (not locally verified against bazel itself -- bazel isn't installed in this sandbox, same real gap as every other manual-build-script-first change in this repo). Real, honest, not done: no wiring into apps/arena's live match loop, no rendering, no AI deciding where to found a town, Walled Hamlet/Jungle Enclave/Blighted Settlement/faction AI/tech tree/corruption mechanic/win condition all explicitly named as open design work in the NORTHSTAR doc, not guessed at. (sess-20260905-0720-ec33e7c5)
- Fountains proceduralized (S376-01): margin from map corner is now a real per-match PRNG output (ARENA_FOUNTAIN_MARGIN_MIN/MAX, computed in arena_set_match_seed) instead of a fixed literal -- founder: 'it seems like we lost fountains, can you add them into the procedural generation of the map?' Shop system fundamentally redesigned (S376-02): ARENA_SHOP_COUNT (6) fully neutral, procedurally placed shops (angular-sector rejection sampling, same exclusion discipline as the S370 jungle) replace the old 2-shops-one-per-team-everything-for-sale design; the 34-item catalog is Fisher-Yates shuffled and split round-robin across the 6 shops per match, so which item is sold where is randomized every game -- founder: 'spawn like 6 shops on the map and divide the items on the pages between them... building towards a specifc kit with items is about strategy and luck.' New PACKET_ARENA_SNAPSHOT_LAYOUT/ArenaSnapshotLayoutMsg wire message (fountain + shop positions/catalogs) since apps/arena_bot deliberately doesn't link the sim and can't independently derive per-match-random positions the way apps/arena does -- also fixed apps/arena_bot's own stale hand-copied fountain/shop position literals (confirmably wrong since the 2026-09-07 9x map widen; bots have likely been walking to the wrong coordinates ever since) and gave the bot real shop-catalog-aware shopping (skips to the next wishlist item some shop actually stocks instead of blindly cycling catalog order). Build-template system removed outright (founder: 'oh yea you can rip out templates, not needed in this version') -- ArenaBuildTemplate/arena_hero_apply_build_template/PACKET_ARENA_APPLY_BUILD_TEMPLATE/the shop UI's build-presets page/build_template_mod.c all deleted; it assumed one shop sold everything, which the redesign made untrue. bash scripts/build.sh + scripts/build_arena.sh + scripts/test_arena.sh all clean (3013 PASS, 0 FAIL). (sess-20260905-0720-ec33e7c5)
- Mandelbrot procedural jungle generation (S370): 32-piece hand-placed obstacle layout stays exactly as-is; 96 new procedurally-generated trees/rocks (ARENA_OBSTACLE_COUNT 32->128) fill the space between bases via Mandelbrot escape-time boundary sampling, seeded fresh per match (isolated xorshift32 PRNG, not libc rand()) so client and server independently compute the identical layout with no wire sync, matching the founder's 'procedurally generate the map for each new game' ask. Real gaps found and fixed along the way, not assumed: ECOWAR's own matchmaker/server never actually had the seed-plumbing protocol.h's own comment described (MatchFoundMsg was pre-DUNGEON_NORTHSTAR, no seed field at all, no --seed argv anywhere) -- built for real: matchmaker generates + passes --seed to the spawned server and MatchFoundMsg.seed to every client, also fixed a pre-existing uninitialized-stack-garbage bug in that same message. Exclusion zones (nodes/graveyards/shops/fountains/camps/other obstacles) keep every base, lane, and objective clear. ArenaSnapshotMsg's obstacle_hp[] split into its own PACKET_ARENA_SNAPSHOT_OBSTACLES packet (same MTU-fragmentation precedent as the earlier hero-chunk split) since 128 obstacles would have reopened that budget. New tests/test_procedural_jungle.c: exclusion-zone, determinism, and per-seed-variety checks. Live-verified end to end with a throwaway matchmaker+server+client round trip (seed generated, matched identically on both received MatchFoundMsg copies, server accepted --seed and started clean) -- production ecowar-matchmaker.service NOT restarted, still running the pre-change binary until a deploy is explicitly requested. bash scripts/build.sh + bash scripts/test_arena.sh both clean (3138 PASS, 0 FAIL). (sess-20260905-0720-ec33e7c5)
- fix(ops): real root cause of "stuck on queueing" for real players found and fixed -- `ecowar-matchmaker.service` had been dead for 5 days (`systemctl --user status` showed `inactive (dead)`, `Result: resources`, journal: "Failed to load environment files: No such file or directory") while `ecowar-bot-pool.service` kept running the whole time against a matchmaker nothing was listening on. Root cause: the unit's `EnvironmentFile=/home/fatbaby/ECOWAR/var/redgarden-iduna-agent.env` was a bare (non-optional) reference to a file that never existed for ECOWAR specifically -- systemd fails a unit's entire activation when a required `EnvironmentFile` is missing, so the matchmaker binary never even ran. Fixed two ways: (1) provisioned a real, genuinely distinct `ECOWAR-BOTS` M2M agent identity in IDUNA (new migration `IDUNA/migrations/truestore/202609110001_ecowar_bots_agent.sql` + `config/agents.json` entry + `cmd/bootstrap`), resolving `README.md`'s own previously-flagged "does ECOWAR report under its own agent identity or REDGARDEN's?" question -- its own, so WOTAN match stats never mix into REDGARDEN's leaderboard -- and wrote the real secret to a new `var/ecowar-iduna-agent.env`; (2) made the `EnvironmentFile=` line optional (leading `-`) so a future missing/misconfigured file degrades only WOTAN match-result reporting, never core matchmaking again. Deployed and live-verified end to end: real matchmaker process running and listening on UDP `0.0.0.0:9779`, log shows `IDUNA agent configured: name=ECOWAR-BOTS ... (WOTAN match-result reporting available)`, and a real two-bot test match (temporary second `red_garden_arena_bot` process alongside the persistent pool bot) queued, matched, spawned a dedicated arena server, both connected, both drafted heroes, and real gameplay ticks were exchanged. Also found and named (not fixed -- needs root, unavailable in this sandbox): `var/matches`/`var/corpus` are owned by a different user (`treeiii`) without world-write permission, so the arena server (now running as `fatbaby`) silently can't write match/AI-corpus logs ("could not open match log ... will not be logged") -- a real, separate, non-blocking gap, logged to backlog for a human with sudo to fix. (sess-20260905-0720-ec33e7c5)
...
- Fixed real 'failed to join match' bug: the manually-started ECOWAR 1v1 matchmaker was missing REDGARDEN_TICKET_SECRET, so it matched real players but the spawned arena server rejected every join (fail-closed). Restarted with the env var set (matching PLAY.bat's own client-side value); verified a real connection now succeeds. Honest limitation: systemd deploy of the already-correctly-configured units isn't possible from this sandbox (no user D-Bus session) -- currently running as a manual background process that won't survive a reboot, real systemd deploy still needed as follow-up. Apple #16426. session: sess-20260825-1938-f6bd411e (sess-20260825-1938-f6bd411e)
